...
class GR4J(object):
    ...
    def __init__(self, area, input_parameters, States, calibration=False, method='PSO', Objective_fun='nse',
        ...
        self.n = None
        self.Date = None
        self.lb = [0, -10, 1, 0.5, 0, 1]
        self.ub = [1500, 5, 500, 4, 1, 10]
    ...
    def InitData(self):
        ...
        self.T = self.df.Temp
        self.E = self.df.E
        self.df.Q = GR4J_.m3_to_mm(self.df.Q, self.Area)
        self.Qobs = self.df.Q
    ...
    def Objective(self, x, *args):
        ...
        self.Qsim = GR4J_.gr4j_cemaneige(self.df, x, self.Spinoff)
    ...
    def run(self):
        ...
        if self.Cal:
            if self.method == 'PSO':
                ...
                self.Qsim = GR4J_.gr4j_cemaneige(self.df, self.parameters.x, self.Spinoff)
                # self.Qsim, self.St = nam_f.nam_method(
                #     self.parameters.x, self.States, self.P, self.T, self.E, self.area, self.Spinoff, Cal=False)
                self.parameters = self.parameters.x
        else:
            self.Qsim = GR4J_.gr4j_cemaneige(self.df, self.initial, self.Spinoff)
            # self.Qsim, self.St = nam_f.nam_method(
    ...
    def interpolation(self):
        self.Qobs = np.asarray(self.Qobs)
...
if __name__ == '__main__':
    ...
    n = GR4J(3060, params, params, calibration=True, method='PSO', Objective_fun='nse', maxiter=10, Spinoff=90)
    n.process_path = r"D:\Hydrology\Code"
    n.Data_file = os.path.join(n.process_path, "L0123002_1984_2012.csv")
    n.run()
    n.draw()
