...
def _rmse(q_rec, q_sim):
    ...
    obs, sim = np.array(q_rec), np.array(q_sim)
    mse = np.nanmean((obs - sim) ** 2)
    # if erro.any < 0:
    #     return(np.nan)
    f = np.sqrt(np.nanmean(mse))
    return f
...
def calibrate(flow, avg_prec, temp, et, p2, init_st=None, ll_temp=None,
    ...
    if x_0 is None:
        ...
        x_0 = (1.06653896e+00, 7.85803777e-01, 1.12456396e+00, 4.00000005e-02
    ...
    par_cal = minimize(_cal_fun, x_0, method='SLSQP', bounds=bounds,
